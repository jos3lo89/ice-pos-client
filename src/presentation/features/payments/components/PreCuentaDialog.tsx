...
const PreCuentaDialog = ({
  ...
  const handlePrint = async () => {
    if (!orderData) {
      toast.error("No se pudo obtener la información de la pre-cuenta");
      return;
    }
    const blob = await pdf(<PreCuentaPdf data={orderData} />).toBlob();
      ...
      <DialogContent className="max-w-2xl w-[95vw] h-[90vh] bg-[#0f172a] border-slate-800 p-0 overflow-hidden shadow-2xl rounded-xl flex flex-col gap-0 transition-all duration-300">
        <DialogHeader className="p-4 flex flex-row items-center justify-between border-b border-white/5 shrink-0">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-lg bg-yellow-500/10 flex items-center justify-center border border-yellow-500/20 shadow-inner">
          ...
          <DialogDescription className="sr-only">
            Previsualización e impresión de la pre-cuenta
          </DialogDescription>
        </DialogHeader>
          ...
          {isLoading ? (
            <div className="flex-1 flex flex-col items-center justify-center p-12 animate-in fade-in zoom-in-95 duration-500">
              <div className="relative mb-6">
                ...
                <LoadingState message="Estructurando datos para la pre-cuenta..." />
              </div>
            </div>
          ) : isError ? (
            <div className="flex-1 flex flex-col items-center justify-center p-12 bg-slate-900/30 backdrop-blur-sm animate-in fade-in duration-500">
              <ErrorState
                ...
                message="Tuvimos problemas para obtener los detalles de la orden."
                onRetry={refetch}
              />
            </div>
          ) : (
            <div className="flex-1 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700">
              ...
              <div className="p-6 bg-slate-900/80 backdrop-blur-md border-t border-white/5 flex flex-wrap items-center justify-between gap-4 shrink-0 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.5)]">
                ...
                <Button
                  variant="default"
                  onClick={handlePrint}
                  className="bg-linear-to-r from-yellow-600 to-amber-600 hover:from-yellow-500 hover:to-amber-500 text-white font-black px-8 h-12 rounded-lg transition-all shadow-xl shadow-yellow-500/20 active:scale-95 group relative overflow-hidden"
                >
                  <div className="absolute inset-0 bg-white/10 translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
                  <Printer className="w-5 h-5 mr-2 relative z-10 group-hover:animate-bounce" />
                  <span className="relative z-10">Imprimir Pre Cuenta</span>
                </Button>
              </div>
            </div>
          )}
...
export default PreCuentaDialog;
